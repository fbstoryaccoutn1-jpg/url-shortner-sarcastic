...
const registerMsgs = [
  ...
  "🤪 Kya soch raha hai? Registration off hai!"
];
...
app.post('/api/shorten', async (c) => {
  ...
  const slug = nanoid(8);
  await c.env.DB.prepare(`INSERT INTO short_links (id, slug, destination, title, description, image_url, clicks, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?)`).bind(nanoid(), slug, destination, title, description, imageUrl, Date.now(), Date.now()).run();
  return c.json({ slug });
});
